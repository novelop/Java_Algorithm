package hyeji_1007;

import java.io.BufferedReader;
import java.io.IOException;
import java.io.InputStreamReader;
import java.util.Arrays;
import java.util.StringTokenizer;

public class Main_2116_주사위쌓기2 {
	
	static int N,Max;
	static int[][] dice;
	static int[] op = {5,3,4,1,2,0}; //각 마주보는 면의 인덱스를 저
	
	public static void main(String[] args) throws NumberFormatException, IOException {
		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
		
		N = Integer.parseInt(br.readLine());
		
		dice = new int[N][6];
		
		for(int i=0; i<N; i++) {
			StringTokenizer st = new StringTokenizer(br.readLine());
			for(int j=0; j<6; j++) {
				dice[i][j] = Integer.parseInt(st.nextToken());
			}
		}
		
		Max = -1;
		
		//윗 면 정하기 
		for(int i=0; i < 6; i++) {
			int max = 0;
			for(int j=0; j<6; j++) { //옆면 중에서 가장 큰 값
				if(j == i || j == op[i]) continue; //윗면, 아랫면 제외
				max = Math.max(max, dice[0][j]);
			}
			up(dice[0][i], max, 1); //윗면의 값과 옆면중에서 가장 큰 값
		}
		System.out.println(Max);
	}

	private static void up(int top, int sum, int cnt) {
		if(cnt == N) {
			Max = Math.max(sum, Max);
			return;
		}
		//현재 주사위에서 top에 해당하는 인덱스를 찾아야함
		int idx = 0;
		for(int i = 0; i < 6; i++) {
			if(dice[cnt][i] == top) {
				idx = i;
				break;
			}
		}
		int nextTop = op[idx]; //현재 주사위의 윗면의 인덱스 결정
		int m = 0;
		for(int j = 0; j < 6; j++) {
			if(j == nextTop || j == idx) continue;
			m = Math.max(m, dice[cnt][j]);
		}
		up(dice[cnt][nextTop], sum+m, cnt+1);
		
	}

}
