...
public class Main_23291_어항정리 {
	...
	private static void gongjungSecond() {
		int ref = N /2 + N / 4;
		for(int j = 1; j >= 0; j--) { //각 리스트의 마지막 부터 수행
			for(int i=0; i< N / 4; i++) { 
	...
	private static void open() {
		...
		for(int i = 0; i < N; i++) {
			...
			for(int j=0; j<arr[i].size(); j++) {
				openarr[idx] = new ArrayList<Integer>();
	...
	private static boolean roll() {
		...
		while(true) {
			if(arr[startIdx].size() != 0) { 
				size = arr[startIdx].size(); //시작인덱스 사이즈 저장
				break;
			}
			startIdx++;
		}
		...
		while(true) {
			//endIdx가 N보다 크거나 같은 경우 이거나 endIdx의 사이즈가 시작인덱스 사이즈랑 다른 경우 
